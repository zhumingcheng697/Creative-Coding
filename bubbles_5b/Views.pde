...
void vigorousModeSlider() {
  strokeWeight(4);
  stroke(0, 0, 80-50*darkMode);
  line(344, 247, 464, 247);
  stroke(hsbColor[0], 80-10*darkMode, 80-20*darkMode);
  line(344, 247, map(vigorousMode, 0, 1, 344, 464), 247);
  noStroke();
  fill(0, 0, 100);
  ellipse(map(vigorousMode, 0, 1, 344, 464), 247, 22, 22);
}
...
void pauseScreen() {
  ...
  if (!(!adjusting && paused && (mousePressed || keyPressed && key == ' ') && mouseX>280 && mouseX<520 && mouseY>155 && mouseY<210)) {
    ...
    textAlign(CENTER, CENTER);
    textSize(28);
    text("You Have Played", 400, 245);
    textSize(27); 
    text(timePlayed, 400, 310); 
    textSize(22); 
    if (count == 1) {
      text(count + " Game", 400, 350);
    } else {
      text(count + " Games", 400, 350);
    }
  }
}
